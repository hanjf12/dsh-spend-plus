...
`;
		 ...
		 * activity streaks. */
		function SummaryCards({ t, totals, callCount, sessionsScanned, currency, billing, budget, overview }) {
...
.mono{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px}
		...
		function Dashboard({ t, data, error, lastUpdated, onRefresh, onOpenDetails, cwd, onCwdChange }) {
			...
			const overview = react_jsx_runtime.jsx(react_jsx_runtime.Fragment, {
				children: [
					react_jsx_runtime.jsx(SummaryCards, { t, totals, callCount: data.callCount, sessionsScanned: data.sessionsScanned, currency, billing: data.billing, budget: data.budget, overview: data.overview }),
		...
		function UsageStatsWidget({ t, query }) {
			...
			react.useEffect(() => {
				...
				if (win !== null && !win.closed && data !== null && data.totals !== undefined) {
					renderDetailsWindow(win, data, data?.currency ?? "USD", t, lastUpdated);
				}
			}, [data, lastUpdated]);
						...
						? react_jsx_runtime.jsx("div", {
							...
							children: [
								...
								error !== null
									...
									: totals === undefined || data.callCount === 0
...
//#region \0dsh-css:C:\deepseekHarness\plugins\dsh-balance-meter\src\client\balance.module.css.mjs
		const css = ".gdQcsW_chip{border:1px solid var(--dsh-color-border,#80808059);background:var(--dsh-color-surface,transparent);height:22px;color:var(--dsh-color-text,inherit);cursor:pointer;white-space:nowrap;border-radius:999px;align-items:center;gap:6px;padding:0 8px;font-size:12px;line-height:1;display:inline-flex;position:relative}.gdQcsW_chip:hover,.gdQcsW_chipOpen{border-color:var(--dsh-color-accent,#50a0ffb3)}.gdQcsW_dot{background:var(--dsh-color-danger,#e5534b);border-radius:50%;flex:none;width:7px;height:7px}.gdQcsW_dotOk{background:var(--dsh-color-success,#2da44e);border-radius:50%;flex:none;width:7px;height:7px}.gdQcsW_details{z-index:40;border:1px solid var(--dsh-color-border,#80808059);background:var(--dsh-color-surface-elevated,#1f1f1f);min-width:140px;color:var(--dsh-color-text,inherit);border-radius:6px;flex-direction:column;gap:2px;padding:6px 8px;font-size:11px;display:flex;position:absolute;top:calc(100% + 6px);left:0;box-shadow:0 4px 12px #0000004d}.gdQcsW_row{justify-content:space-between;align-items:center;gap:12px;display:flex}.gdQcsW_rowRight{align-items:center;gap:4px;display:inline-flex}.gdQcsW_sep{opacity:.45}.gdQcsW_cost{opacity:.85}.gdQcsW_costRow{border-top:1px solid var(--dsh-color-border,#80808040);justify-content:space-between;align-items:center;gap:12px;margin-top:4px;padding-top:4px;display:flex}";
		const tagId = "dsh-spend-plus/balance.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-spend-plus";
			tag.dataset.pluginCss = tagId;
			tag.textContent = css;
			document.head.appendChild(tag);
		}
		var balance_module_css_default = {
			"chip": "gdQcsW_chip",
			"chipOpen": "gdQcsW_chipOpen",
			"cost": "gdQcsW_cost",
			"costRow": "gdQcsW_costRow",
			"details": "gdQcsW_details",
			"dot": "gdQcsW_dot",
			"dotOk": "gdQcsW_dotOk",
			"row": "gdQcsW_row",
			"rowRight": "gdQcsW_rowRight",
			"sep": "gdQcsW_sep"
		};
		//#endregion
		...
		*/
		...
		/** Dictionary namespace owned by this plugin. */
		const BALANCE_NS = "balance";
		/** Required services: slots for the composer-dock entry, locale for the copy. */
		const balanceInject = ["slots", "locale", "connection"];
		/**
		* Register the balance chip into the composer dock band.
		* @param ctx - client root context.
		*/
		function applyBalance(ctx) {
			ctx.effect(() => ctx.locale.register(BALANCE_NS, { zh: balanceZh, en: balanceEn }), "dsh-spend-plus: balance dictionaries");
			ctx.inject(["slots", "conversation"], (scope) => {
				scope.effect(() => scope.slots.register({
					...
					locale: BALANCE_NS,
					inject: () => ({})
				}, BalanceDockEntry), "dsh-spend-plus: balance chip");
			});
		}
		//#endregion
